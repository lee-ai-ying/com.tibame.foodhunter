...
fun IsOpenNow(openingHours: String): Boolean {
    ...
    for (dayHours in dayOfHoursList){
        if (dayHours.startsWith(currentDay)){
            val timeRange = dayHours.substringAfter(":").replace("–", "-").split("-").map{it.trim()}
            if (timeRange.size == 2 ) {
                val openingTime =
                    LocalTime.parse(timeRange[0], DateTimeFormatter.ofPattern("HH:mm"))
                var closeTime = LocalTime.parse(timeRange[1], DateTimeFormatter.ofPattern("HH:mm"))
                if (timeRange[1] == "00:00") {
